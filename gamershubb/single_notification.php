...
<a href="<?php echo $link ?>" style="text-decoration: none; color:black;">
	<div id="notification" style="background-color: <?= $color ?>; width: 388px; border: 11px solid #1a1a1a; margin-left: -10px;">
		...
		<?php
			if(is_array($actor) && is_array($owner)){
				...
				echo "<img src='$image' style='width:25px;margin:4px;float:left;' />";
				...
				if($notif_row['content_type'] == "post"){
					...
					if($content_row['has_image']){
						...
						if(file_exists($content_row['image']))
						{
							...
							echo "<img src='$post_image' style='width:31px;float:right;' />";
						}
 					}else{
					...
					echo $notif_row['content_type'];
					echo "
						<span style='float:right;font-size:px;color:#888;display:inline-block;margin-right:10px;'>'".htmlspecialchars(substr($content_row['post'],0,50))."'</span>
					";
				}
				...
				echo "<br>
					...
					<span style='font-size:11px;color:red;display:inline-block;margin-right:10px;'>$date</span>
				";
			}
		?>
	</div>
