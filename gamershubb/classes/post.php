	...
	public function create_post($userid, $data, $files)
	{
		...
		if(!empty($data['post']) || !empty($files['file']['name']) || isset($data['is_profile_image']) || isset($data['is_cover_image']))
		{
				...
				if(!empty($files['file']['name']))
				{
					...
					$folder = "uploads/" . $userid . "/";
						...
						if(!file_exists($folder))
						{
							...
							file_put_contents($folder . "ghindex.php", "");
						}
			...
			if($this->error == ""){
				...
				$query = "insert into posts (userid,postid,post,image,has_image,is_profile_image,is_cover_image,parent,likes,comments,tags) values ('$userid','$postid','$post','$myimage','$has_image','$is_profile_image','$is_cover_image','$parent','$likes','$comments','$tags')";
				$DB->save($query);
